...
class Slideshow {
  ...
  setupSlideshow() {
    ...
    let $counterContainer = $(
      "<div> of  " + this.totalItems + " </div>"
    ).insertAfter(this.$slideshow);
    this.$counter = $("<b>" + this.counter + "</b>").prependTo(
      $counterContainer
    );

    this.$nextButton = $("<button id='nextButton'>Next</button>").click(
      this.buttonHandler.bind(this)
    );
    this.$prevButton = $("<button id='prevButton'>Prev</button>").click(
      this.buttonHandler.bind(this)
    );
    this.$startStopButton = $(
      "<button id='startStopButton'>Stop</button>"
    ).click(this.buttonHandler.bind(this));
